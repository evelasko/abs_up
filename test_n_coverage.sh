...
run_tests() {
    if [[ -f "pubspec.yaml" ]]; then
        ...
        flutter pub global run remove_from_coverage:remove_from_coverage -f coverage/lcov.info -r '.g.dart$|.freezed.dart$'
    else
...
run_report() {
    if [[ -f "coverage/lcov.info" ]]; then
        ...
        genhtml -o coverage/html coverage/lcov-final.info
        open coverage/html/index-sort-l.html
    else
